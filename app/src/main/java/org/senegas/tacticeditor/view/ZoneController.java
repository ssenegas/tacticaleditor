...
public class ZoneController extends JPanel {
  ...
  private enum ButtonModel {
	AREA1("1", PitchZone.AREA1, false), AREA2("2", PitchZone.AREA2, false), AREA3("3", PitchZone.AREA3, false),
	AREA4("4", PitchZone.AREA4, false), AREA5("5", PitchZone.AREA5, false), AREA6("6", PitchZone.AREA6, false),
	AREA7("7", PitchZone.AREA7, false), AREA8("8", PitchZone.AREA8, false), AREA9("9", PitchZone.AREA9, false),
	AREA10("10", PitchZone.AREA10, false), AREA11("11", PitchZone.AREA11, false), AREA12("12", PitchZone.AREA12, false),
	KICKOFF_OWN("Kickoff own", PitchZone.KICKOFF_OWN, true), KICKOFF_DEF("Kickoff def", PitchZone.KICKOFF_DEF, true),
	GOALKICK_OWN("Goalkick own", PitchZone.GOALKICK_OWN, true),
	GOALKICK_DEF("Goalkick def", PitchZone.GOALKICK_DEF, true), CORNER1("Corner1", PitchZone.CORNER1, true),
	CORNER2("Corner2", PitchZone.CORNER2, true), CORNER3("Corner3", PitchZone.CORNER3, true),
  ...
  private JPanel buildResumeGamePanel() {
	...
	ButtonModel.stream().filter(ButtonModel::isResumeGame).forEach(bm -> grid.add(this.buttons.get(bm)));
	return grid;
  }
  ...
  private JPanel buildZoneSelectorPanel() {
	...
	ButtonModel.stream().filter(Predicate.not(ButtonModel::isResumeGame)).forEach(bm -> grid.add(this.buttons.get(bm)));
	return grid;
  }

  public void enableDisableButtons() {
	final boolean b = this.model.getTatic() != null;
	this.buttons.values().stream().forEach(tb -> tb.setEnabled(b));
  }
