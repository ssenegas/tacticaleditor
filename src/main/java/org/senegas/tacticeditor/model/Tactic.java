...
//	}
  ...
  private static Map<PitchZone, Map<Integer, Point>> read(InputStream is) {
	final Map<PitchZone, Map<Integer, Point>> positions = new EnumMap<>(PitchZone.class);

	try (BufferedInputStream bis = new BufferedInputStream(is);
	    ByteArrayOutputStream baos = new ByteArrayOutputStream();) {

	  final byte[] bytes = new byte[Tactic.NUMBER_OF_PLAYERS * PitchZone.values().length * 2 * 2]; // 10 players x 20
	                                                                                               // areas x position x
	                                                                                               // and y (2 bytes per
	                                                                                               // value, so 4)
	  ...
	  // wrap byte array to a list of points, resulting in a list of two hundred
	  // points (20 pitch zones with 10 positions)
	  final List<Point> points = extractPoints(baos.toByteArray());

	  // form ten groups of twenty points. For each player, his position for each
	  // pitch zone.
	  final List<List<Point>> splittedPositions = chunk(points, PitchZone.values().length).stream()
	  ...
	  for (final PitchZone pitchZone : PitchZone.values()) {
		...
		IntStream.range(0, Tactic.NUMBER_OF_PLAYERS).boxed()
		    ...
		    .forEach(p -> {
		      ...
		      final Map.Entry<Integer, Point> entry = extractPlayerPitchZonePosition(splittedPositions, pitchZone, p);
  ...
  private static Map.Entry<Integer, Point> extractPlayerPitchZonePosition(List<List<Point>> splittedPositions,
      PitchZone pitchZone, Integer index) {
	final List<Point> pos = splittedPositions.get(index);
	final Integer squadNumber = Tactic.SQUAD_NUMBERS.get(index);
